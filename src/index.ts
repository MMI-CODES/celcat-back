...
const CACHE_TTL_SECONDS = parseInt(process.env.CACHE_TTL_SECONDS || '600', 10); // Default to 10 minutes
 ...
 */
async function fetchIcalData(groupId: string): Promise<string> {
    const cachedData = icalCache.get<string>(groupId);
    if (cachedData) {
        return cachedData;
    }
...
function parseIcalData(icalData: string): CalendarEvent[] {
    ...
    const events: CalendarEvent[] = Object.values(calendar) // calendar values are of type any
        .filter(event => event.type === 'VEVENT')
        .map((event: any) => { // Use 'any' here as we've already filtered for VEVENT
 ...
 */
async function getTimetable(groupId: string, startDate: Date, endDate?: Date): Promise<CalendarEvent[]> {
